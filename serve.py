...
routeEmbedding = SentenceTransformerEmbedding(EmbeddingConfig(name=OPEN_AI_EMBEDDING_MODEL))
productRoute = Route(name=PRODUCT_ROUTE_NAME, samples=productsSample)
chitchatRoute = Route(name=CHITCHAT_ROUTE_NAME, samples=chitchatSample)
semanticRouter = SemanticRouter(routeEmbedding, routes=[productRoute, chitchatRoute])
...
def handle_query():
    ...
    if guidedRoute == PRODUCT_ROUTE_NAME:
        ...
        combined_information = f"Hãy trở thành chuyên gia thời trang tư vấn bán hàng cho một sàn thương mại điện tử thời trang qui tụ nhiều thương hiệu. Câu hỏi của khách hàng là: {query}\nTrả lời câu hỏi dựa vào các thông tin sản phẩm dưới đây: {source_information}. Nếu bạn làm tốt tôi sẽ thưởng cho bạn 200000 VNĐ vào cuối ngày."
        data.append({
            "role": "user",
            "parts": [
                {
